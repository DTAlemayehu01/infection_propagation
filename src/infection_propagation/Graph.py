...
class Graph(object):
    ...
    def __check_node_existence(self, src, dst, log=False):
        if log:
            print(self.vertices())
            print(src)
        if not set(src).issubset(set(self.vertices())):
            raise ValueError(f"A source node {src} is not in the graph")
        if not set(dst).issubset(set(self.vertices())):
            raise ValueError(f"A destination node {dst} is not in the graph")

    # Make sure graph is using scipy.stats library
    # TODO: Use heapq
    # TODO: Refactor
    # TODO: Make multidimensional, like birectional
    def simulate_gossip_rv(self, src, dst, log=False, preserve_times=False):
        ...
        while True:
            current_tick_infected = [
                infected
                for infected in self._infected.keys()
                if self._infected[infected] == True
            ]
            ...
            for infected in current_tick_infected:
                ...
                for infect_idx, weight in enumerate(self._adjency_matrix.loc[infected]):
                    ...
                    if (
                        not self._simulated[(infected, new_infection)]
                        and weight != np.inf
                    ):
                        ...
                        if preserve_times == True:
                            ...
                            if self._directed == False:
                                adj_matrix_dupe.loc[new_infection, infected] = (
                                    edge_delay
                                )
            ...
            self._adjency_matrix.loc[current_tick_infected] = self._adjency_matrix.loc[
                current_tick_infected
            ].sub(min_infect_time)
            if self._directed == False:
                self._adjency_matrix.loc[min_edge[1], min_edge[0]] = np.inf
                self._adjency_matrix.loc[:, current_tick_infected] = (
                    self._adjency_matrix.loc[:, current_tick_infected].sub(
                        min_infect_time
                    )
                )
    ...
    def birectional_simulate_gossip_rv(self, src, dst, log=False):
        ...
        while any(infection_frontier_backward_heaps) and infection_frontier_forwards:
            ...
            for u in self.vertices():
                ...
                if item != np.inf:
                    ...
                    for i, dist_dst in enumerate(dist_dsts):
                        if (
                            u in dist_dst.keys()
                            and dist_src[v_src] + item + dist_dst[u] < mu[i]
                        ):
            ...
            for i, heap in enumerate(infection_frontier_backward_heaps):
                ...
                for u in self.vertices():
                    ...
                    if item != np.inf:
                        ...
                        if (
                            u in dist_src.keys()
                            and dist_dst[v_dst] + item + dist_src[u] < mu[i]
                        ):
            ...
            if all(
                dist_src[v_src] + min_backward_tops[i] >= mu[i] for i in range(len(dst))
            ):
                break
    ...
    def construct_time_from_path(self, src, dst):
        ...
        if son == src:
            ...
            raise RuntimeError(
                "src does not match identified source, double check algorithm"
            )
    ...
    def sim_all(self, reset=False):
        ...
        for i in self.vertices():
            for j in self.vertices():
                if (
                    not self._simulated[(i, j)]
                    and self._adjency_matrix.loc[i, j] != np.inf
                ):
        ...
        while pq_src and pq_dst:
            ...
            for u in self.vertices():
                ...
                if item != np.inf:
                    ...
                    if (
                        u in dist_dst.keys()
                        and dist_src[v_src] + item + dist_dst[u] < mu
                    ):
            ...
            for u in self.vertices():
                ...
                if item != np.inf:
                    ...
                    if (
                        u in dist_src.keys()
                        and dist_dst[v_dst] + item + dist_src[u] < mu
                    ):
    ...
    def make_edge_set(self, edge_json):
        ...
        for key, value in edge_json.items():
            ...
            edge_tuple = (int(edges[0]), int(edges[1]), distribution)
...
def erdos_renyi_simulation_trial(
    n, p, src, dst, iters=10**3, bidirectional_opt=True, log=False, **kwargs
):
...
def random_tree_simulation_trial(
    n, branch, src, dst, iters=10**3, bidirectional_opt=True, log=False, **kwargs
):
    path_counts = defaultdict(lambda: 0)
    path_times = defaultdict(list)
    for i in range(iters):
...
def fixed_tree_simulation_trial(
    n, branch, src, dst, iters=10**3, bidirectional_opt=True, log=False, **kwargs
):
    path_counts = defaultdict(lambda: 0)
    path_times = defaultdict(list)
